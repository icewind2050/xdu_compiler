...
3. 假定$e_1,e_2$为$\Sigma$上的正规式,表示的正规集为$L(e_1),L(e_2)$则
   ...
   4. 有限使用以上三个步骤定义的表达式称为$\Sigma$上的正规式
...
由CFG G所产生的语言$L(G)$被定义为$$L(G)=\{\omega\mid S\overset{*}{\rArr}\omega \enspace and \enspace \omega\in T^*\}$$
...
如果出现这样的产生式$$A\rarr\alpha\beta_1\mid\alpha\beta_2\mid\cdots\mid\alpha\beta_n\gamma$$可以将其这样改写为$$\begin{align*}
    A\rarr&\alpha A'\mid\gamma\\
    A'\rarr&\beta_1\mid\beta_2\mid\cdots\mid\beta_n\\
\end{align*}$$
...
E&\rarr E+T\mid E-T\mid T\\
T&\rarr T*F\mid T/F\mid T \; mod\; F\mid F\\
...
消去左递归和左因子得到了递归下降分析所需的文法$$\begin{align*}
    ...
    T&\rarr FT'\\
    T'&\rarr *FT'\mid /FT'\mid mod\; FT'\mid\epsilon\\
    F&\rarr (E)\mid i\mid num\\
